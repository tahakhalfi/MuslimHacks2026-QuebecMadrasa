...
export default function HomePage() {
  ...
  return <main className="real-landing">
    <header className="real-nav-wrap">
      <nav className="real-nav" aria-label="Main navigation"><Link href="/" className="real-brand"><img className="site-logo-image" src="/ui/logo-madrasa-quebec.png" alt="Madrasa Québec Network" /></Link><div className="real-nav-links">{t.nav.map((label, index) => <a key={label} href={anchors[index]}>{label}</a>)}</div><div className="real-nav-actions"><span className="real-nav-lang-desktop">{languageSwitch}</span><Link href="/parent" className="real-button real-button-outline">{t.login}</Link><Link href="/parent/onboarding" className="real-button real-button-dark">{t.start}</Link><button type="button" className="real-nav-toggle" aria-expanded={menuOpen} aria-controls="real-mobile-menu" aria-label={locale === "fr" ? (menuOpen ? "Fermer le menu" : "Ouvrir le menu") : (menuOpen ? "Close menu" : "Open menu")} onClick={() => setMenuOpen((open) => !open)}>{menuOpen ? "✕" : "☰"}</button></div></nav>
      {menuOpen && <div className="real-nav-mobile-panel" id="real-mobile-menu">{t.nav.map((label, index) => <a key={label} href={anchors[index]} onClick={() => setMenuOpen(false)}>{label}</a>)}<div className="real-nav-mobile-lang">{languageSwitch}</div><div className="real-nav-mobile-actions"><Link href="/parent" className="real-button real-button-outline" onClick={() => setMenuOpen(false)}>{t.login}</Link><Link href="/parent/onboarding" className="real-button real-button-dark" onClick={() => setMenuOpen(false)}>{t.start}</Link></div></div>}
    </header>
    <section className="real-hero" id="familles"><div className="real-hero-copy"><p className="real-eyebrow">{t.eyebrow}</p><h1>{t.title}</h1><p className="real-lead">{t.lead}</p><div className="real-trust-row">{t.trust.map((label, index) => <span key={label}><b>{["⚜", "▤", "♙", "♟"][index]}</b><strong>{label}</strong></span>)}</div><div className="real-hero-actions"><Link href="/parent/onboarding" className="real-button real-button-dark real-button-large">✦ &nbsp; {t.create}</Link><a href="#fonctionnement" className="real-button real-button-outline real-button-large">▷ &nbsp; {t.how}</a></div></div><div className="real-hero-art"><img src="/ui/family-hero.png" alt={locale === "fr" ? "Une famille apprend ensemble à la maison au Québec" : "A family learning together at home in Quebec"} /></div></section>
    ...
    <section className="real-pricing" id="tarifs"><p className="real-eyebrow">{t.priceEyebrow}</p><h2>{t.priceTitle}</h2><p>{t.priceText}</p><Link href="/parent/onboarding" className="real-button real-button-dark">{t.createShort}</Link></section>
